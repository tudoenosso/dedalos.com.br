...
class GmpNewUser
{
    ...
    {
        $valEmail = new \Gmp\Models\helper\GmpValEmail();
        $valEmail->validateEmail($this->data['email']);

        $valEmailSingle = new \Gmp\Models\helper\GmpUserSingle();
        $valEmailSingle->validateUserSingle($this->data['email']); 

        $valSenha = new \Gmp\Models\helper\GmpValSenha();
        $valSenha->validateSenha($this->data['senha']);       

        if (($valEmail->getResult()) and ($valSenha->getResult()) and ($valEmailSingle->getResult())) {
            $this->addUser();
        } else {
            $this->result = false;
        }
    ...
    {
        unset($this->data['confirma_senha']);       
        $this->data['senha'] = password_hash($this->data['senha'], PASSWORD_DEFAULT);        
        $this->data['fk_situacao'] = 1;
        $this->data['fk_niv_acesso'] = 2;
        $this->data['cadastro'] = date("Y-m-d H:i:s");
       
       

        $createUser = new \Gmp\Models\helper\GmpCreate();
        $createUser->exeCreate("gmp_usuarios", $this->data);

        if ($createUser->getResult()) {
            Alerta::alert("Usuário cadastrado com sucesso!", "sucesso");
            //$_SESSION['msg'] = "<p class='alert-success'>Usuário cadastrado com sucesso!</p>";
            $this->result = $createUser->getResult();
        } else {
            Alerta::alert("Erro 018: Não foi possível cadastrar o usuário. Tente novamente ou contate o administrador!", "erro");
